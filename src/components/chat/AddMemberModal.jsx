...
export default function AddMemberModal({ existingMemberIds, onClose, onAdd }) {
  ...
  const candidates = friends.filter((f) => !existingMemberIds.includes(f.id));
  const filteredFriends = candidates.filter((f) => (f.name || "").toLowerCase().includes(query.trim().toLowerCase()));
  ...
  const submit = async () => {
    ...
    try {
      ...
      console.error("Không thể thêm thành viên:", err);
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-sm bg-zm-card rounded-2xl border border-zm-border shadow-2xl glow-violet flex flex-col max-h-[80vh]">
        <div className="flex items-center justify-between px-4 py-3 border-b border-zm-border shrink-0">
          <h2 className="font-bold text-lg">Thêm thành viên</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Đóng"
        ...
        <div className="p-4 flex flex-col gap-3 overflow-y-auto">
          ...
          {loading ? (
            ...
            <p className="text-sm text-zm-muted text-center py-4">Tất cả bạn bè đã có trong nhóm.</p>
